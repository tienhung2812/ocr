...
class TextCombinator:
    ...
    def prepare_category_data(self):
        ...
        if os.path.exists(data_json_path):
            ...
            self.data['arrange_sentence'] = np.nan
            self.data['sentence'] = np.nan
            self.data['category'] = np.nan
            self.data['cate_conf'] = np.nan
            for index, row in self.data.iterrows():
                if row['parrent'] <0:
                    cate_data = current_data[str(index)]
                    sentence = self.arrange_text(index,row,self.get_text_file_pandas(index))
                    self.data.loc[index,'sentence'] = cate_data['sentence']
                    self.data.loc[index,'arrange_sentence'] = str(sentence)
                    self.data.loc[index,'category'] = cate_data['cate']
                    self.data.loc[index,'cate_conf'] = str(cate_data['cate_conf'])
            return True
        return False

    def group_by_cate(self):
        prepare_cate_data_sucess =  self.prepare_category_data()
        category = ['brand_name', 'info', 'index', 'content', 'total', 'thank_you']
        if prepare_cate_data_sucess:
            category_data = {}
            for col in category:
                data = self.data[self.data['category'] == col]
                category_data[col] = {}
                for index, row in data.iterrows():
                    category_data[col][index] = {
                        "arrange_sentence":row["arrange_sentence"],
                        "sentence":row["sentence"],
                        "category":row["category"],
                        "cate_conf":row["cate_conf"],
